...
# A naive algorithm which uses statistical analysis to obtain solutions
def median_avg(cities, num_stations):
	# obtain a list of the average distance to all other cities for each city
	averages = {}
	for i, city in enumerate(cities):
		city_avg = np.average(city)
		averages[i] = city_avg

	# sort the list of average distances
	sorted_avgs = {key: val for key, val in sorted(
		averages.items(), key=lambda ele: ele[1], reverse=False)}

	medians = []
	# fill a quarter of the solution with the lowest average distanced cities
	for key, val in list(sorted_avgs.items())[:num_stations//4]:
		medians.append(key)
		del sorted_avgs[key]
	# fill the rest of the solution with the cities with the median distance to all other cities
